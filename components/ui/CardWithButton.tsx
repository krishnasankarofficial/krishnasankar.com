...
const CardWithButton = ({
  ...
  return (
    <div 
      className={`relative w-full ${main ? 'h-[350px] md:h-[450px]' : 'min-h-[50px] lg:min-h-[215px]'} bg-gradient-to-t from-slate-900
      to-slate-800 bg-opacity-80 hover:from-cyan-500 hover:to-green-500 hover:text-black rounded-3xl shadow-lg xl:p-6 p-4 
      ...
      {img && (
        <div className="flex items-start justify-start z-0">
          <Image 
            ...
            height={300}
            alt="Service image"
            className="object-contain opacity-100 slatescale -mt-12"
          />
        </div>
      )}
      {/* Card Content */}
      <h3 className={`font-bold mb-2 font-raleway ${main ? 'text-xl lg:text-3xl' : 'text-lg md:text-xl'} z-10`}>{title}</h3>
      <p className="text-slate-300 group-hover:text-slate-900 text-sm md:text-md z-10 hyphens-auto break-words pr-4">
        {description}
      </p>
